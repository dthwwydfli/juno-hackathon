...
/** Full-screen white result after save when the new medicine has potential interactions. */
export function AddInteractionResult({
  newMed,
  existing,
  interaction,
  extraCount = 0,
  ...
  return (
    <PhoneFrame label="Potential interaction">
      <StatusBar />
      <SubHeader title="Potential interaction" onBack={onBack} right={<HomeButton />} />

      <div className="screen-body add-ix-result">
        <div className="add-ix-result-inner">
          <div className="warn-halo">
            <Icon name="warning" size={44} strokeWidth={1.8} />
          </div>
          <h2 className="warn-title add-ix-result-title">Potential interaction</h2>
          <p className="warn-sub add-ix-result-sub">
          ...
          <div className="warn-pair add-ix-result-pair">
          ...
          <div className="add-ix-result-card">
            <button type="button" className="add-ix-readmore" onClick={onReadMore}>
              <span className="r">
          ...
          <div className="warn-actions add-ix-result-actions">
            <button type="button" className="btn primary" onClick={onReadMore}>
              Review interaction
              <Icon name="chevron" size={18} strokeWidth={2} />
            </button>
            <button type="button" className="btn warn-ghost" onClick={onAddAnyway}>
              Add to my list anyway
            </button>
          </div>

          {extraCount > 0 && onViewAll && (
            <button type="button" className="add-ix-view-all" onClick={onViewAll}>
              View all on Interactions
            </button>
          )}
